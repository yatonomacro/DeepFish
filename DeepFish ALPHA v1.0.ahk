...
;     Precision mode's control method (PD steering with velocity tracking and stopping
;     distance) is adapted from IRUS IDIOTPROOF by AsphaltCake.
...
ProfileFields := [ ["General","AutoLowerGraphics","true","bool"]
                  ...
                  , ["Shake","ShakeMode","Click","shakemode"]
                  ...
                  , ["Minigame","ControlMode","Precision","controlmode"]
                  ...
                  , ["Tuning","StableRightMultiplier","2.1","num"]
...
if (MacroRunning)
	{
	MacroRunning := false
	GuiControl,, StartStopDisplay, ▶ Start
...
MacroRunning := true
LastAutoStop := 0
GuiControl,, StartStopDisplay, ⏹ Stop
goto StartMacroFlow
...
GateMsg .= "DeepFish is free. If it helps you, please consider subscribing — "
GateMsg .= "the channel opens when you continue.`n`n"
...
for i, f in ProfileFields
	...
	if (type = "bool")
		...
		if (val = "Classic")
			val := "Manual"
		if (val != "Manual" and val != "Precision")
			{
			ConfigWarnings .= key . "`n"
			val := def
			}
...
BarPresent() {
	global BarLeftColor, BarRightColor, BarScanRow
...
Gui, Add, Text, x14 y36 w300 h16 HwndhSubtitle, Version 1.0 • Made by Yato
...
Gui, Add, Text, x0 y108 w132 h420 HwndhSidebarBG
...
Gui, Add, Text, x10 y152 w112 h36 +0x100 vNavShake gSidebarClick HwndhNavShake, 🐚 Shake
SpecialBrushes[hNavShake] := {brush: hBrushPanel, text: ColorTextBGR}
Gui, Add, Text, x10 y192 w112 h36 +0x100 vNavMinigame gSidebarClick HwndhNavMinigame, 🎮 Minigame
SpecialBrushes[hNavMinigame] := {brush: hBrushPanel, text: ColorTextBGR}
Gui, Add, Text, x10 y232 w112 h36 +0x100 vNavTuning gSidebarClick HwndhNavTuning, ⚙️ Tuning
SpecialBrushes[hNavTuning] := {brush: hBrushPanel, text: ColorTextBGR}
Gui, Add, Text, x10 y272 w112 h36 +0x100 vNavAbout gSidebarClick HwndhNavAbout, ℹ️ About
...
GeneralNumDefs := [ ["Restart delay (ms)","RestartDelay"]
                  , ["Cast hold duration (ms)","HoldRodCastDuration"]
                  , ["Bobber wait delay (ms)","WaitForBobberDelay"] ]
...
ShakeNumDefs := [ ["Fish bar tolerance","FishBarColorTolerance"]
                 , ["Click failsafe (s)","ClickShakeFailsafe"]
...
MinigameFieldDefs := [ ["Bar calc failsafe (s)","BarCalculationFailsafe",1
	...
	, ["Arrow tolerance","ArrowColorTolerance",1
...
for i, d in ColorFieldDefs
	...
	MinigameCtrls.Push(hS)
	Y += RH
	}

Gui, Add, Text, x%CX% y%Y% w%LW% h20 HwndhL, Bar color tolerance:
MinigameCtrls.Push(hL)
Gui, Add, Edit, x%IX% y%Y% w150 h20 vBarColorTolerance gQueueAutoSave HwndhC +Number, % BarColorTolerance
MinigameCtrls.Push(hC)
ThemedCtrls.Push(hC)
Gui, Font, s9 Bold c%ColorAccent%, Segoe UI
Gui, Add, Text, x492 y%Y% w16 h20 Center +0x100 +0x200 HwndhQ, ?
Gui, Font, s9 Norm c%ColorMuted%, Segoe UI
MinigameCtrls.Push(hQ)
IconTips[hQ] := "How closely the picked colours must match.`n`nHIGHER = more forgiving, needed if the rod`nshifts hue or the screen shakes.`nTOO HIGH = matches the wrong thing.`n`nAround 12-20 works for most rods."
Y += RH + 10
...
Gui, Add, Text, x%CX% y%Y% w372 h60 HwndhL, Drag a box around the fishing bar. The snip opens zoomed in. Press a "Set ..." button`, then click that part of the image.
MinigameCtrls.Push(hL)
...
TuningCtrls.Push(hL)
ControlModeChoice := (ControlMode = "Manual") ? 1 : 2
Gui, Add, DropDownList, x%IX% y%Y% w150 h100 vControlMode gControlModeChanged Choose%ControlModeChoice% HwndhC, Manual|Precision
TuningCtrls.Push(hC)
ThemedCtrls.Push(hC)
Gui, Font, s9 Bold c%ColorAccent%, Segoe UI
Gui, Add, Text, x492 y%Y% w16 h20 Center +0x100 +0x200 HwndhQ, ?
...
Gui, Font, s9 Norm cFFFFFF, Segoe UI
Gui, Add, Text, x%CX% y%Y% w372 h18 HwndhAbtVer, Version 1.0 • Made by Yato
...
AboutCredits .= "• AsphaltCake — Precision mode control method,`n"
AboutCredits .= "   adapted from IRUS IDIOTPROOF.`n"
...
GuiControl,, NavGeneral, % (TargetSection = "General" ? "▸ 🎣 General" : "   🎣 General")
GuiControl,, NavShake, % (TargetSection = "Shake" ? "▸ 🐚 Shake" : "   🐚 Shake")
GuiControl,, NavMinigame, % (TargetSection = "Minigame" ? "▸ 🎮 Minigame" : "   🎮 Minigame")
GuiControl,, NavTuning, % (TargetSection = "Tuning" ? "▸ ⚙️ Tuning" : "   ⚙️ Tuning")
GuiControl,, NavAbout, % (TargetSection = "About" ? "▸ ℹ️ About" : "   ℹ️ About")
...
GuiControl,, ProfileSelect, %ProfileNames%
GuiControl, ChooseString, ProfileSelect, %ActiveProfile%
...
for i, f in ProfileFields
	...
	if (type = "bool")
		{
		chkval := val ? 1 : 0
		GuiControl,, %key%, %chkval%
		}
	else if (type = "shakemode" or type = "controlmode")
		{
		GuiControl, ChooseString, %key%, %val%
		}
	else
		GuiControl,, %key%, %val%
	}
...
GuiControl,, SaveStatusText, ✅ Saved
...
GuiControlGet, NewProfileChoice,, ProfileSelect
...
Gui, Pick:Add, Button, x170 y%PickBtnY2% w90 h28 gPickClose, Close
