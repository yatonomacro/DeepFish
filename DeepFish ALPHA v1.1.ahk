...
Gui, Add, Text, x14 y36 w300 h16 HwndhSubtitle, Version 1.1 • Made by Yato
...
Gui, Add, Text, x10 y312 w112 h36 +0x100 vNavAbout gSidebarClick HwndhNavAbout, ℹ️ About
...
Gui, Add, Text, x%CX% y%Y% w%LW% h20 HwndhL, Fish line tolerance:
MinigameCtrls.Push(hL)
Gui, Add, Edit, x%IX% y%Y% w150 h20 vFishBarColorTolerance gQueueAutoSave HwndhC +Number, % FishBarColorTolerance
MinigameCtrls.Push(hC)
ThemedCtrls.Push(hC)
Gui, Font, s9 Bold c%ColorAccent%, Segoe UI
Gui, Add, Text, x492 y%Y% w16 h20 Center +0x100 +0x200 HwndhQ, ?
Gui, Font, s9 Norm c%ColorMuted%, Segoe UI
MinigameCtrls.Push(hQ)
IconTips[hQ] := "How closely a pixel must match the fish line`ncolour to count as the fish.`n`nHIGHER = more forgiving. Raise this if the`nminigame is never detected.`nTOO HIGH = scenery gets mistaken for the fish."
Y += RH + 10
...
Gui, Font, s9 Norm cFFFFFF, Segoe UI
Gui, Add, Text, x%CX% y%Y% w372 h18 HwndhAbtVer, Version 1.1 • Made by Yato
...
for i, f in ProfileFields
	...
	else if (type = "shakemode" or type = "controlmode" or type = "castmode")
		{
...
Gui, Pick:Add, Button, x12 y%PickBtnY3% w90 h28 gPickClose, Close
