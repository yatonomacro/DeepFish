...
for i, f in ProfileFields
	...
	if (type = "bool")
		...
		{
		if (val != "None" and val != "Pinion Aria" and val != "Requiem" and val != "Darkheart" and val != "Bellona's Waraxe" and val != "Dreambreaker" and val != "Tranquility" and val != "Noiseform" and val != "Verdant Oath" and val != "Ruinous Oath" and val != "Luminescent Oath")
			{
			ConfigWarnings .= key . "`n"
			val := def
			}
...
NoiseGimmickTarget() {
	...
	if (!pend)
		{
		if (NoiseZOK and !NoiseZPrevOK)
		return -1
		}
	if (!NoiseZOK)
		{
		if (((gnow - NoisePendT) / QpcFreq) > 1.50)
		return -1
		}
	gz := -1
...
Gui, Add, Text, x14 y36 w300 h16 HwndhSubtitle, Version 1.3 • Made by Yato
...
SpecialRodChoice := (SpecialRod = "Pinion Aria") ? 2 : ((SpecialRod = "Requiem") ? 3 : ((SpecialRod = "Darkheart") ? 4 : ((SpecialRod = "Bellona's Waraxe") ? 5 : ((SpecialRod = "Dreambreaker") ? 6 : ((SpecialRod = "Tranquility") ? 7 : ((SpecialRod = "Noiseform") ? 8 : ((SpecialRod = "Verdant Oath") ? 9 : ((SpecialRod = "Ruinous Oath") ? 10 : ((SpecialRod = "Luminescent Oath") ? 11 : 1)))))))))
Gui, Add, DropDownList, x%IX% y%Y% w150 h100 vSpecialRod gSpecialRodChanged Choose%SpecialRodChoice% HwndhC, None|Pinion Aria|Requiem|Darkheart|Bellona's Waraxe|Dreambreaker|Tranquility|Noiseform|Verdant Oath|Ruinous Oath|Luminescent Oath
MinigameCtrls.Push(hC)
...
Gui, Font, s9 Norm cFFFFFF, Segoe UI
Gui, Add, Text, x%CX% y%Y% w372 h18 HwndhAbtVer, Version 1.3 • Made by Yato
