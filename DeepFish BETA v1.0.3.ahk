...
; A cast bar on screen while a fight is ending proves the hold was charging a cast (holding the mouse
; outside a fight charges one), so releasing now fires a stray cast.  Green cap AND white fill below
; it, twice ~40 ms apart, so a green object in the world cannot trigger it.
CastChargeCheck:
...
; Splitbranch Twig: the rod offers two fish and waits for a click. Either is fine, so the left one is
; taken. Checked at most every 300 ms, and only while waiting for a fight - never during one.
TwigMaybePick:
if (!MacroRunning)
	return
...
ProfFieldOk(type, ByRef val) {
	...
	if (type = "specialrod")
		...
		if (val = "Pinion Aria")
			val := "Pinions Aria"
		return (val = "None" or val = "Pinions Aria" or val = "Requiem" or val = "Darkheart" or val = "Bellona's Waraxe" or val = "Dreambreaker" or val = "Tranquility" or val = "Noiseform" or val = "Verdant Oath" or val = "Lullaby" or val = "Castbound" or val = "Remembrance" or val = "MiguRod" or val = "Halibut Harpoon" or val = "Splitbranch Twig")
		}
...
; a swing = the fill has been up (>= 90 %) and is back down (<= 10 %)
if (PCFill >= 90)
	PCSawTop := true
...
PerfectAcquireGreen() {
	...
	; row by row: the longest green run must be cap-sized (6..59 px) AND have the cast bar under it -
	; green text, foliage and UI pass the first test and fail the second.
	gRow := -1
	gl := -1
	gr := -1
	Loop, 2
		...
		while (y < y1)
			...
			; wrong size, or nothing bar-shaped under it: skip every row this green thing occupies
			y++
...
PerfectTrackFrame() {
	...
	; the cap does not move during a cast: look where it was last frame first, then the top 110 rows
	scanTo := (h < 110) ? h : 110
	gRow := -1
	gl := -1
	gr := -1
	...
	; the white fill, read across the 12-px band at the cap centre (as before): its top is the highest
	; white pixel in the band, its bottom the last white row with gaps of up to 16 rows bridged
	cx := (gl + gr) // 2
	x0 := cx - 6
	if (x0 < 0)
		x0 := 0
...
; ---- pixel finder -------------------------------------------------------------------------------------
; prm = [tB, tG, tR, tol, mode, invert] as Int32: a pixel passes when it is within tol of the target, or -
; mode 1 - it is plainly green (g>80, g>r+22, g>b+22), or - mode 2 - plainly white (r,g,b>200, spread<32).
; PcFind returns the index of the first pixel that passes (invert = 0) or fails (invert = 1), -1 if none.
...
; longest run of passing pixels on one row, bridging gaps of up to 2 px (what the old scanner did at x step 2)
...
; Is there a cast bar under this green cap?  At six depths the column must be bar-like inside (dark
; track or bright fill, low saturation) and show a luminance edge at both of the cap's x limits.
; The world behind can be anything, so the edges are contrast, not an absolute colour test.
PcCapVerify(gRow, lo, hi) {
	...
	for i, fr in [0.020, 0.040, 0.070, 0.100, 0.145, 0.190]
		...
		tried++
		p := pPCFullBits + (y * w * 4)
		; inside samples sit 2 px in from the outline (the fill is shaded 167..243 left to right)
		li := (lo + 2 < cx) ? lo + 2 : cx
		ri := (hi - 2 > cx) ? hi - 2 : cx
...
; Is the Splitbranch Twig "Choose one!" prompt on screen?  One capture covering 0.17-0.58 H of the
; middle fifth of the window holds both signals: the white prompt text across the top of the strip,
; and the two fish name labels across the bottom (bright text with a dark outline, one cluster each
; side of centre).  Returns the click targets in client coordinates - the x of each label's centroid,
; and the labels' y - so a different aspect ratio still lands on the options.
TwigPrompt(ByRef lcx, ByRef rcx, ByRef labY) {
	...
	; the white prompt text, across the top of the strip (every 2nd pixel is plenty)
	pbH := Round(h * 0.17)
	white := 0
	y := 0
	while (y < pbH)
	...
	; the two name labels: bright text with a dark outline beside it, one cluster each side of centre
	yb := Round(h * 0.878)
	half := w // 2
	lN := 0
	rN := 0
...
; x64 machine code for PcFindPixAhk (same self-test-or-fallback scheme as LineHitsFn)
PcFindPixFn() {
...
; Pinions Aria reads the catch progress bar, which sits ~4.3 % H under the reel bar: capture down to
; it (a BitBlt costs the same whatever its height).  Row index is relative to the strip top.
...
TrackEdgesVisible() {
	global UiTopRows, UiBotOff, UiEdgeH
	tM := UiEdgeBandMax(2, UiTopRows - 3, 2, 8, rt)
	bM := UiEdgeBandMax(UiBotOff + 2, UiEdgeH - 3, 2, 8, rb)
...
UiStartGate() {
	...
	; no track edges: an unknown skin gets the benefit of the doubt after 0.5 s; once this rod has
	; shown its edges in a fight, a colour hit without them waits 4 s (a phantom never lasts that long)
...
UiEdgeReset() {
	...
	global UiEdgeTY, UiEdgeBY, UiPeakT, UiPeakB, UiThr
	UiState := 0
	UiPhantomT := 0
	UiGoneT := 0
	UiGreenT := 0
...
UiEdgeStart() {
	global UiState, UiLearnT, UiLearnN, UiAccT, UiAccB, UiGoneT, UiGreenT, UiEdgeOn
	UiEdgeReset()
	if (!UiEdgeOn)
		{
		UiState := 3
...
UiEdgeTick() {
	...
	if (UiState = 1)
		...
		if (UiLearnN >= 3 and ((unow - UiLearnT) / QpcFreq) >= 0.5)
			{
			bT := -1
			bTv := -1
			for k, v in UiAccT
			...
			UiPeakT := (bT >= 0) ? (bTv / UiLearnN) : 0
			UiPeakB := (bB >= 0) ? (bBv / UiLearnN) : 0
...
LineScan() {
	...
	; Pinions Aria at lower resolutions: the marker is 4-6 px wide and often yields ONE edge line, not
	; the pair - find it by colour instead (magenta below the track, cyan above) and keep the bar,
	; whose edge lines are fine.  Other rods keep the pair rule exactly as before.
	; ... and at 720p the bar's own rim is a 4-6 px edge pair too (fish-outside state), which the
	; pair rule picked as the fish - so for Pinions the colour comes FIRST and the pair is the fallback.
	pinCol := false
	if (SpecialRod = "Pinions Aria" and PinMarkerX(fcx))
		{
		pinCol := true
		; the colour says WHICH lines are the marker; the edge pair straddling it gives the exact centre
		ci := 1
	...
	if (others.Length() >= 2)
		...
		if (pinCol)
			{
			; Pinions with the marker known and the track ends excluded: what is left is the bar's rim
			; lines (outer, plus inner in the fish-outside state), so the bar is the outermost pair.  If a
			; stray line widened it by more than a quarter, keep the pair matching the last width instead.
			oLft := others[1]
...
; Pinions Aria fish marker by colour: a magenta column 3.8 % H below the strip top (under the track)
; and a cyan one 0.9 % H above it - both rows are in the capture already (the UI band above, the
; progress-bar rows below).  Returns the marker's centre x, strip-relative.
PinMarkerX(ByRef cx) {
	...
	; the magenta part of the marker sits 2.6-4.2 % H below the strip's top depending on the setup
	; (a 720p tester: 2.6-3.9 %), so a band of rows is tried, then the cyan part above the track
...
; Pinions Aria.  Measured (Alvin's manual session, 2560x1440): notes fall straight down at ~0.60 H/s
; and land ~1.4 s after they appear; a note counts when the bar covers it as it reaches the bar's top
; edge, anywhere across the bar's width; catch progress rises ~0.13/s with the fish inside the bar
; and FALLS ~0.09/s with it outside.  So: stay on the fish; place the bar so the note lands inside it
; while the fish stays inside too whenever the geometry allows; when it does not, leave just in time
; and only if the progress can afford the trip; come straight back once the note has landed.
PinionPlan(fishX, noteX, ByRef isNote) {
	...
	if (reach < 4)
		reach := 4
	; ---- Resonance state: the fish is locked to the bar and notes fall two at a time; until a note
	; is missed the only job is the notes.  Detected from behaviour, no UI read: the fish marker
	; stays on the bar's centre while the bar travels, or two notes on screen on three scans running.
	if (fishX >= 0)
		{
	...
	; committed: hold the cover position until the note has landed, whatever the scanner says now
	if (PnState = 1)
		{
		if (pnNow >= PnImpactAt)
			PnState := 0
			...
			; a fresh scan of the same note still falling: keep holding until it has really landed
			if (NoteFresh and NoteLockY >= 0 and noteX >= 0 and Abs(noteX - PnNoteX) <= 60)
				{
				vF := (PnFallN >= 3 and PnFall > 0) ? PnFall : (WindowHeight * 0.60)
				ttiF := (NoteH - NoteLockY) / vF
				landAt := pnNow + Round((ttiF + 0.15) * QpcFreq)
	...
	; fall speed from consecutive captures of the same note
	if (NoteFresh and NoteLockY >= 0)
		{
	...
	; time until the note reaches the bar, extrapolated from the last capture
	tti := (NoteH - NoteLockY) / vFall - ((pnNow - PnPrevT) / QpcFreq)
	if (tti < -0.05)
		return fishX
	; where the bar must be: on the fish when that keeps the note 30 % of the width inside the edge,
	; otherwise centred on the note (a trip is needed anyway, so take the most tolerant position)
	...
	; out of reach even at full speed with no settling: do not chase it
	if (tGo + 0.15 > tti + 0.05)
		{
		PnWhy := "skip: too far"
		return fishX
		}
	; leave with 0.9 s in hand for the controller to settle - for a far note that means leaving the
	; moment it is seen (the settle allowance decides how early to leave, never whether to go)
	tTravel := tGo + 0.90
	; a trip away from the fish must be affordable (no trip when the fish stays inside)
	...
	; not yet: stay on the fish until the last moment
	if (tti > tTravel + 0.10)
		{
	...
	PnNoteX := noteX
	PnCoverX := cover
	PnImpactAt := pnNow + Round((tti + 0.15) * QpcFreq)
	isNote := (cover != fishX and Abs(barC - cover) <= NoteHoldZone(RunWidth))
	PnWhy := "go " . Round(Abs(cover - barC)) . "px"
	return cover
}

; The catch progress bar under the reel bar: white = done, purple = remaining.  Read from the strip
; already captured this loop with the native finder (a few calls), so it costs microseconds.
PinProgress() {
...
Gui, Add, Text, x22 y42 w320 h16 HwndhSubtitle, Version 1.0.3  •  Made by Yato
...
SpecialRodChoice := (SpecialRod = "Pinions Aria") ? 2 : ((SpecialRod = "Requiem") ? 3 : ((SpecialRod = "Darkheart") ? 4 : ((SpecialRod = "Bellona's Waraxe") ? 5 : ((SpecialRod = "Dreambreaker") ? 6 : ((SpecialRod = "Tranquility") ? 7 : ((SpecialRod = "Noiseform") ? 8 : ((SpecialRod = "Verdant Oath") ? 9 : ((SpecialRod = "Lullaby") ? 10 : ((SpecialRod = "Castbound") ? 11 : ((SpecialRod = "Remembrance") ? 12 : ((SpecialRod = "MiguRod") ? 13 : ((SpecialRod = "Halibut Harpoon") ? 14 : ((SpecialRod = "Splitbranch Twig") ? 15 : 1)))))))))))))
Gui, Add, DropDownList, x%IX% y%Y% w%IW% h260 vSpecialRod gSpecialRodChanged Choose%SpecialRodChoice% HwndhC, None|Pinions Aria|Requiem|Darkheart|Bellona's Waraxe|Dreambreaker|Tranquility|Noiseform|Verdant Oath|Lullaby|Castbound|Remembrance|MiguRod|Halibut Harpoon|Splitbranch Twig
MgAdd("mode", hC)
ThemedCtrls.Push(hC)
Gui, Font, s9 Bold c%ColorAccent%, Segoe UI
Gui, Add, Text, x%QX% y%Y% w16 h20 Center +0x100 +0x200 HwndhQ, ?
...
Gui, Font, s9 Norm cFFFFFF, Segoe UI
Gui, Add, Text, x%CX% y162 w%TXW% h18 HwndhAbtVer, Version 1.0.3  •  Made by Yato
...
RtY := 198
; ---- the unlock card: the rest of the macro stays locked until this box is ticked ----
